    ...
    <div class="material-header">
        ...
        <div class="material-header-actions">
            ...
            @if (auth()->user()->role !== 'guest')
                <button type="button" class="btn-action success" x-data
                    x-on:click="$dispatch('open-modal', 'importModal')">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th style="width: 8%; text-align: center; cursor: pointer;"
                                    wire:click="sort('unrestricted_use_stock')">
                                ...
                                <th style="width: 15%; text-align: right; cursor: pointer;"
                                    wire:click="sort('total_nilai')">
                                    <div
                            ...
                            @foreach ($materials as $index => $material)
                                <tr>
                                    ...
                                    <td style="text-align: center;">
                                        <span
                                            class="stock-badge">{{ number_format($material->unrestricted_use_stock, 0, ',', '.') }}</span>
                                    </td>
                                    ...
                                    @if (auth()->user()->role !== 'guest')
                                        <td>
                                            <div class="action-buttons">
                                                <a href="{{ route('material.show', $material) }}" wire:navigate
                                                    class="action-btn view" title="Detail">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                {{-- Edit button removed --}}
                                                <a href="{{ route('material.generate-barcode', $material) }}"
                                                    class="action-btn barcode" title="Generate Barcode">
                ...
                <div class="alert-purity info">
                    ...
                    @if ($search)
                        ...
                        <button wire:click="$set('search', '')"
                            style="background: none; border: none; font-weight: 600; cursor: pointer; color: inherit; text-decoration: underline;">Reset
                            pencarian</button>
                    @else
                        Belum ada data material. <a href="{{ route('material.create') }}" wire:navigate
                            style="color: inherit; font-weight: 600;">Tambah material pertama</a>.
                    @endif
                </div>
            </div>
        @endif
