    ...
    <div class="bg-white rounded-xl">
        <table class="w-full text-sm">
            <tbody class="divide-y divide-gray-100">
                ...
                @if (!$isSapDone && $suratJalan->jenis_surat_jalan === 'Normal')
                    <tr>
    ...
    <div class="bg-white rounded-xl">
        @if ($isSecurity)
                ...
                <div class="space-y-3">
                    <div class="grid grid-cols-1 gap-1">
                        ...
                        <input type="text" name="kendaraan"
                            class="form-input text-sm w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500"
                    ...
                    <button type="button" id="btnSaveKendaraan"
                        class="w-full mt-2 bg-blue-600 text-white text-xs font-bold py-2 rounded-lg hover:bg-blue-700 transition shadow-md">
                        SIMPAN
                    </button>
                </div>
            </form>
            ...
            <div class="mt-4 text-xs text-gray-500 space-y-1 pl-1">
                ...
                <p>Approved: <span class="font-medium text-gray-700">{{ $suratJalan->approver->nama ?? '-' }}</span>
                </p>
            </div>
        @else
            <table class="w-full text-sm">
                <tbody class="divide-y divide-gray-100">
                    <tr>
                        <td class="py-2 text-gray-500 font-medium w-32">Kendaraan</td>
                        <td class="py-2 text-gray-900">: {{ $suratJalan->kendaraan ?? '-' }}</td>
                    </tr>
                    <tr>
                        <td class="py-2 text-gray-500 font-medium">No Polisi</td>
                        <td class="py-2 text-gray-900">: {{ $suratJalan->no_polisi ?? '-' }}</td>
                    </tr>
                    <tr>
                        <td class="py-2 text-gray-500 font-medium">Pengemudi</td>
                        <td class="py-2 text-gray-900">: {{ $suratJalan->pengemudi ?? '-' }}</td>
                    </tr>
                    <tr>
                        <td class="py-2 text-gray-500 font-medium">Dibuat Oleh</td>
                        <td class="py-2 text-gray-900">: {{ $suratJalan->creator->nama ?? '-' }}</td>
                    </tr>
                    <tr>
                        <td class="py-2 text-gray-500 font-medium">Disetujui Oleh</td>
                        <td class="py-2 text-gray-900">: {{ $suratJalan->approver->nama ?? '-' }}</td>
                    </tr>
                    <tr>
                        <td class="py-2 text-gray-500 font-medium">Keterangan</td>
                        <td class="py-2 text-gray-900">: {{ $suratJalan->keterangan ?? '-' }}</td>
                    </tr>
                </tbody>
            </table>
        @endif
...
@if ($suratJalan->foto_penerima)
    <div class="mb-6 bg-gray-50 rounded-xl p-4 border border-gray-100">
        ...
        <div class="flex flex-col items-center">
            <a href="{{ asset('storage/' . $suratJalan->foto_penerima) }}" target="_blank"
                class="block overflow-hidden rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition">
                <img src="{{ asset('storage/' . $suratJalan->foto_penerima) }}" alt="Foto Penerima"
                    class="max-w-xs object-cover hover:scale-105 transition duration-300" style="max-height: 200px;">
            </a>
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-4 py-3 text-center">
                            @if (!$detail->is_checked)
                                <input type="checkbox" name="detail_ids[]" value="{{ $detail->id }}"
                                    class="check-item w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500 cursor-pointer">
                            @else
                    ...
                    @else
                        <td class="px-4 py-3 font-mono text-xs text-blue-600">
                            {{ $detail->material->material_code ?? '-' }}</td>
                        <td class="px-4 py-3 font-medium text-gray-900">
                            {{ $detail->material->material_description ?? '-' }}</td>
                        <td class="px-4 py-3 text-gray-900 font-bold">{{ $detail->quantity }}</td>
...
@if ($isSecurity && $hasUnchecked)
    <div class="mt-4 flex justify-end">
        <button type="button" id="btnChecked"
            class="btn-primary flex items-center gap-2 shadow-lg shadow-blue-500/30 px-6 py-2 rounded-xl">
...
<script>
    ...
    $(document).off('click', '#btnSaveKendaraan').on('click', '#btnSaveKendaraan', function() {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                btn.prop('disabled', false).html(originalText);
                Swal.fire('Error!', 'Gagal menyimpan data: ' + (xhr.responseJSON?.message ||
                    'Unknown error'), 'error');
            }
        });
    });
