...
@section('content')
    <div class="space-y-6">
        ...
        <div class="card border border-gray-100 shadow-xl shadow-gray-200/50">
            ...
            <!-- Tab Content -->
            <div class="p-6">
                <!-- Tab: Pengaturan Perusahaan (Removed) -->
                <div id="content-company" class="tab-content hidden">
                    <div class="p-4 text-center text-gray-400">
                        <i class="fas fa-info-circle text-2xl mb-2"></i>
                        <p>Tidak ada pengaturan perusahaan yang tersedia.</p>
                    </div>
                </div>
                        ...
                        <table class="table-purity w-full">
                            <thead>
                                <tr class="bg-gray-50">
                                    ...
                                    <th class="px-4 py-3 text-xs font-bold text-gray-500 uppercase tracking-wider">Email
                                    </th>
                                    ...
                                    <th class="px-4 py-3 text-xs font-bold text-gray-500 uppercase tracking-wider">Bergabung
                                    </th>
                                    <th
                                        class="px-4 py-3 text-xs font-bold text-gray-500 uppercase tracking-wider text-right">
                                        Aksi</th>
                                </tr>
                            </thead>
                                ...
                                @foreach ($users as $user)
                                    <tr class="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td class="px-4 py-3">
                                            @if ($user->role == 'admin')
                                                <span
                                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">Admin</span>
                                            @elseif($user->role == 'security')
                                                <span
                                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">Security</span>
                                            @elseif($user->role == 'petugas')
                                                <span
                                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">Petugas</span>
                                            @else
                                                <span
                                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">{{ ucfirst($user->role) }}</span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="flex justify-end items-center gap-2">
                                                <button
                                                    class="p-2 rounded-lg text-yellow-600 hover:bg-yellow-50 transition-colors btn-edit-user"
                                                    data-id="{{ $user->id }}" data-name="{{ $user->nama }}"
                                                    data-email="{{ $user->email }}" data-role="{{ $user->role }}"
                                                    data-unit-id="{{ $user->unit_id }}" title="Edit">
                                                ...
                                                @if ($user->id !== auth()->id())
                                                    <button
                                                        ...
                                                        onclick="confirmDeleteUser({{ $user->id }})" title="Hapus">
                                                        <i class="fas fa-trash"></i>
                                                    </button>
                                                    <form id="delete-user-{{ $user->id }}"
                                                        action="{{ route('settings.users.delete', $user->id) }}"
    ...
    <div id="modalAddUser" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="fixed inset-0 bg-gray-800 bg-opacity-40 transition-opacity" onclick="closeModal('modalAddUser')">
            </div>

            <div
                class="inline-block bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full relative">
                <form action="{{ route('settings.users.store') }}" method="POST">
                    ...
                    <div class="bg-gradient-to-r from-green-500 to-green-600 px-6 py-4">
                        <div class="flex items-center justify-between">
                            ...
                            <button type="button" class="text-white hover:text-gray-200 transition-colors"
                                onclick="closeModal('modalAddUser')">
                        ...
                        <div>
                            ...
                            <select name="unit_id" class="form-input" required>
                                @foreach ($units as $unit)
                                    <option value="{{ $unit->id }}">{{ $unit->name }} ({{ $unit->plant }} /
                                        {{ $unit->storage_location }})</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3">
                        <button type="button"
                            class="px-4 py-2 rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-100 transition-colors"
                            onclick="closeModal('modalAddUser')">
                            Batal
                        </button>
                        <button type="submit"
    ...
    <div id="modalEditUser" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="fixed inset-0 bg-gray-800 bg-opacity-40 transition-opacity" onclick="closeModal('modalEditUser')">
            </div>
                ...
                <form id="formEditUser" action="" method="POST">
                    ...
                    <div class="bg-gradient-to-r from-yellow-500 to-yellow-600 px-6 py-4">
                        <div class="flex items-center justify-between">
                            ...
                            <button type="button" class="text-white hover:text-gray-200 transition-colors"
                                onclick="closeModal('modalEditUser')">
                                <i class="fas fa-times text-xl"></i>
                            </button>
                        </div>
                    </div>
