...
<body class="antialiased">
            ...
            <div class="sidebar-inner">
                ...
                <nav class="sidebar-nav">
                    @php
                        ...
                        $inspectionGroupOpen =
                            request()->routeIs('material.pemeriksaanFisik') ||
                            request()->routeIs('berita-acara.*') ||
                            request()->routeIs('sap-check.*');
                        $mrwiGroupOpen = request()->routeIs('material-mrwi.*') || request()->routeIs('surat.masa');
                        $isSecurityCikarang =
                    ...
                    @if (auth()->user()->role !== 'security' && !$isSecurityCikarang)
                        <div class="nav-item">
                            <a href="{{ route('material.index') }}" wire:navigate
                                class="nav-link {{ request()->routeIs('material.index') ? 'active' : '' }}">
                                <span class="nav-icon">
                                    <i class="fas fa-cubes"></i>
                                </span>
                                <span>Saldo Material</span>
                            </a>
                        </div>
                    @endif
                    ...
                    @if (in_array(auth()->user()->role, ['admin', 'petugas', 'guest']) && !$isSecurityCikarang)
                        <div class="nav-item">
                            <a href="{{ route('material-masuk.index') }}" wire:navigate
                                class="nav-link {{ request()->routeIs('material-masuk.*') ? 'active' : '' }}">
                                <span class="nav-icon">
                    ...
                    @if (in_array(auth()->user()->role, ['admin', 'guest', 'petugas', 'security', 'satpam']))
                        <div class="nav-item">
                            <a href="{{ route('surat-jalan.index') }}" wire:navigate
                    ...
                    @if (in_array(auth()->user()->role, ['admin', 'petugas']))
                        <div class="nav-item">
                            <a href="{{ route('material.history') }}" wire:navigate
                                class="nav-link {{ request()->routeIs('material.history') ? 'active' : '' }}">
                                <span class="nav-icon">
                        ...
                        @if (in_array(auth()->user()->role, ['admin', 'petugas', 'guest']) && !$isSecurityCikarang)
                            <div class="nav-item nav-group {{ $mrwiGroupOpen ? 'open' : '' }}">
                                <button type="button"
                                    class="nav-link nav-group-toggle {{ $mrwiGroupOpen ? 'active' : '' }}"
                                    aria-expanded="{{ $mrwiGroupOpen ? 'true' : 'false' }}">
                                    <span class="nav-icon">
                                ...
                                <div class="nav-submenu">
                                    <a href="{{ route('material-mrwi.stock', ['category' => 'standby']) }}"
                                        wire:navigate
                                        class="nav-link nav-sublink {{ request()->routeIs('material-mrwi.stock') ? 'active' : '' }}">
                                        <span>Saldo MRWI</span>
                                    </a>
                                    <a href="{{ route('material-mrwi.items') }}" wire:navigate
                                        class="nav-link nav-sublink {{ request()->routeIs('material-mrwi.items') ? 'active' : '' }}">
                                        <span>Daftar Material</span>
                                    </a>
                                    <a href="{{ route('material-mrwi.index') }}" wire:navigate
                                        class="nav-link nav-sublink {{ request()->routeIs('material-mrwi.index') ? 'active' : '' }}">
                                        <span>Input Material MRWI</span>
                                    </a>
                                    <a href="{{ route('material-mrwi.history') }}" wire:navigate
                                        class="nav-link nav-sublink {{ request()->routeIs('material-mrwi.history') ? 'active' : '' }}">
                            ...
                            <div class="nav-item nav-group {{ $inspectionGroupOpen ? 'open' : '' }}">
                                ...
                                <div class="nav-submenu">
                                    @if (in_array(auth()->user()->role, ['admin', 'admin_induk']))
                                        <a href="{{ route('sap-check.index') }}" wire:navigate
                                            class="nav-link nav-sublink {{ request()->routeIs('sap-check.*') ? 'active' : '' }}">
                                            <span class="nav-icon">
                                    ...
                                    @endif
                                    <a href="{{ route('material.pemeriksaanFisik') }}" wire:navigate
                                        class="nav-link nav-sublink {{ request()->routeIs('material.pemeriksaanFisik') ? 'active' : '' }}">
                                        <span class="nav-icon">
                                            <i class="fas fa-clipboard-check"></i>
                                        </span>
                                        <span>Pemeriksaan Fisik</span>
                                    </a>
                                    <a href="{{ route('berita-acara.index') }}" wire:navigate
                                        class="nav-link nav-sublink {{ request()->routeIs('berita-acara.*') ? 'active' : '' }}">
                                        <span class="nav-icon">
    ...
    <div id="userDropdownMenu"
        class="hidden fixed bg-white rounded-xl shadow-lg border border-gray-100 py-2 w-48 z-50">
